...
Intuitively a r.v is a numerical representation of the outcomes of a random experiment. For example if the random experiment is tossing a coin, we can map the outcomes to a r.v $X$ that takes two possible values: 0 for \say{head} and 1 for \say{tail}. That way we mapped the outcomes of the random experiment to something that we can work with!\\
...
The physical meaning of a PMF is the probability that a r.v $X$ will take the value $x$ after the execution of a random experiment ($P_{X}(X=x)$). The term \say{mass} helps to get the intuition since the physical mass is conserved as is the total probability for all hypothetical outcomes $x$. \\
...
Notice that the variance is \textbf{not} linear, hence $Var(X+Y) \neq Var(X) + Var(Y)$. However in the case where $X$ and $Y$ are independent the equality holds. \\
...
\subsubsection{Bernoulli Distribution - Bern(p)}
...
In other words is a sequence of $n$ experiments following a Bernoulli distribution, thus the parametrization by $n$ and $p$. Since the only possible outcomes of a Bernoulli distribution is either 0 or 1, we can formally think of a binomial distribution r.v $Y$ as the sum of $n$ Bernoulli distribution $X_{i}$: $Y=X_{1} + X_{2} + \ldots + X_{n}$. \\
...
\subsubsection{Poisson Distribution - Pois($\lambda$)}
...
where $\lambda$ is actually both the expected value and the variance of the distribution (as we will show in a while).\\
...
In other words, Poisson distribution is a large number of successive Bernoulli trials with very small probability $p$ i.e a binomial distribution with $n \to \infty$ and $p \to 0$ while $\lambda = n p$ is held constant. \\
...
Thus indeed, by taking the limit of a binomial distribution for $n \to \infty$ we end up with a Poisson distribution.\\
...
Hence we showed that the parameter $\lambda$ of Pois($\lambda$) is both the expected value and the variance of the distribution. \\
...
\subsubsection{Geometric Distribution - Geo(p)}
...
An example that a geometric distribution can be used is in the case where an ordinary die is thrown repeatedly until the first time a \say{1} appears. The probability distribution of the number of times it is thrown is supported on the infinite set ${1,2,3, \ldots}$ and is a geometric distribution with $p$ = 1/6. \\
...
\subsubsection{Hypergeometric Distribution - Hypergeometric(N, K, n)}
...
\subsubsection{Negative Binomial Distribution - NB(r,p)}
...
Once again the conservation of density in the continuous case gives a kind of \say{physical} meaning to $f_{X}$. \\
...
\subsubsection{Continuous Uniform Distribution - Unif(a,b)}
...
\bse \label{eq:uniform_continuous}
    f_{X}(x) =
    \begin{cases*}
      c & for $x \in [a,b]$ \\
      0 & otherwise
...
Given that a PDF must satisfy (\ref{eq:density}), we can actually compute the constant $c$ since:
{\setlength{\jot}{10pt}
\begin{align*}
...
Hence, we showed that the constant $c$ is actually the length of the range that the PDF is not 0. By substituting $c$ back to (\ref{eq:uniform_continuous}), we get the final form of a  continuous uniform distribution:
...
\begin{figure}[H]
\includegraphics[scale=0.14]{images/uniform.png}
\centering
\caption {PDF of Unif(a,b)}
\end{figure}

\vspace{10pt}

For the expected value of a  continuous uniform distribution, straight from the definition (\ref{eq:expected_continuous}) we get:
\begin{align*}
...
For the variance of a  continuous uniform distribution we will use (\ref{eq:variance}) so first we calculate $E[X^2]$:
\begin{align*}
...
Substituting $E[X^2]$ and $E[X]^2$ to (\ref{eq:variance}) we get:
\begin{align*}
...
\begin{figure}[H]
\includegraphics[scale=0.24]{images/normal.png}
\centering
\caption {PDF of N($\mu$, $\sigma^2$)}
\end{figure}
...
For the expected value of a normal distribution, straight from the definition (\ref{eq:expected_continuous}) we get:
{\setlength{\jot}{11pt}
\begin{align*}
    E[X] &= \int_{-\infty}^{+\infty} x  f_{X}(x) dx \\
         ...
         &= \frac{1}{\sqrt{2\pi\sigma^2}} \int_{-\infty}^\infty (\sqrt{2} \sigma t + \mu) e^{-t^2} d (\sqrt{2} \sigma t) && \text{ \Big($t = \frac{x-\mu}{\sqrt{2}\sigma}$\Big) }\\
         ...
         &= \frac{1}{\sqrt{\pi}} \Big( \sqrt{2} \sigma \cdot 0 + \mu \cdot \sqrt{\pi} \Big) && \text{(Gaussian integrals)} \\
         &= \frac{1}{\sqrt{\pi}} \cdot (\mu \cdot \sqrt{\pi})\\
         &= \mu
\end{align*}}

For the variance of a normal distribution we will use (\ref{eq:variance}) so first we calculate $E[X^2]$:
{\setlength{\jot}{11pt}
\begin{align*}
    E[X^2] &= \int_{-\infty}^{+\infty} x^2  f_{X}(x) dx \\
         ...
         &= \frac{1}{\sqrt{2\pi\sigma^2}} \int_{-\infty}^\infty (\sqrt{2} \sigma t + \mu)^2 e^{-t^2} d (\sqrt{2} \sigma t) && \text{ \Big($t = \frac{x-\mu}{\sqrt{2}\sigma}$\Big) }\\
           ...
           &= \frac{1}{\sqrt{\pi}} \Big( 2 \sigma^2 \cdot \frac{\sqrt{\pi}}{2} + 2 \sqrt{2} \sigma \mu \cdot 0  + \mu^2 \cdot \sqrt{\pi} \Big) && \text{(Gaussian integrals)} \\
...
So we proved that the parameters of the N($\mu$, $\sigma^2$), $\mu$ and $\sigma^2$ are actually the expected value and variance of the distribution (hence the naming). \\
...
\subsubsection{Standard Normal Distribution - N(0,1)}

In the special case where $\mu=0$ and $\sigma^2=1$ the corresponding normal distribution $N(0,1)$ takes the special name of standard normal distribution (or z-distribution). Remember from the graph of a normal distribution, that the bell curve has a maximum at the expected value (in $N(0,1)$ case at 0), and since the variance is equal to 1 (hence also the standard deviation), each extra unit away from the mean is an extra unit of standard deviation (we use standard deviation since it has same units as the mean). For  $\mu=0$ and $\sigma^2=1$ the corresponding PDF (\ref{eq:normal}) reads:

\bse \label{eq:standard_normal}
    f_{X}(x) = \frac{1}{\sqrt{2\pi}} e ^ {-\frac{x^2}{2}}
\ese

\vspace{5pt}

Remember that from the definition of a PDF (\ref{eq:pdf}), specifically for a standard normal distribution we have:
\bse
...
\begin{figure}[H]
\includegraphics[scale=0.45]{images/standardnormal.png}
\centering
\caption {PDF of N(0,1)}
\end{figure}
...
\subsubsection{Exponential Distribution - Expo($\lambda$)}
...
\begin{figure}[H]
\includegraphics[scale=1.3]{images/exponential.png}
\centering
\caption {PDF of Expo($\lambda$)}
\end{figure}
...
\subsubsection{Chi-Squared Distribution - $\chi^2(k)$}
...
\begin{figure}[H]
\includegraphics[scale=0.3]{images/chi.png}
\centering
\caption {PDF of $\chi^2(k)$}
\end{figure}
...
\subsubsection{Student's t-Distribution - $t(\nu)$}
...
\begin{figure}[H]
\includegraphics[scale=0.17]{images/t.png}
\centering
\caption {PDF of $t(\nu)$}
\end{figure}
...
\subsubsection{Beta Distribution - Beta($\alpha, \beta$)}

Beta distribution Beta($\alpha$, $\beta$) is a family of continuous probability distributions defined on the interval [0, 1] parametrized by two positive shape parameters, denoted by $\alpha$ and $\beta$, that appear as exponents of the random variable and control the shape of the distribution.\\
...
\centering
\caption {PDF of Beta($\alpha$, $\beta$)}
\end{figure}

\vspace{10pt}

Observe that $B(\alpha, \beta)$ is not just  one probability distribution, but a family of probability distributions since for different values of $\alpha$ and $\beta$ we end up with different distributions.\\
...
\subsubsection{Gamma Distribution - Gamma($\alpha$, $\beta$)}

Gamma distribution parametrized by two positive shape parameters $alpha$ and $\beta$ and denoted by Gamma($\alpha$, $\beta$) is a family of continuous probability distributions defined on the interval [0, $\infty$).\\
...
\begin{figure}[H]
\includegraphics[scale=0.2]{images/gamma.png}
\centering
\caption {PDF of $\Gamma(\alpha, \beta)$}
\end{figure}
...
Subsequently, we can define the concept of a \say{joint probability mass function} and a \say{joint probability density function} for a collection of discrete and continuous r.v's respectively. More specifically:
...
\bse \label{eq:independentjoint}
    P_{X_{1}, \ldots, X_{n}}(X_{1} = x_{1}, \ldots, X_{n} = x_{n}) = \prod_{i=1}^{n} P_{X_{i}}(X_{i} = x_{i})
\ese

\vspace{5pt}

Notice that the individual r.v's $\{ X_{1}, \ldots, X_{n} \}$ can follow different distributions. This is why the term $P_{X_{i}}$ appears in (\ref{eq:independentjoint}). However, more often than not we will be dealing with the case where all  $\{ X_{1}, \ldots, X_{n} \}$ individually follow the same distribution $P_{X_{1}} = P_{X_{2}} = \ldots = P_{X_{n}} = P_{X}$. In this case we formally say that we are dealing with \say{independent identical distributed random variables} or i.i.d r.v's. This abbreviation is used a lot in statistics since many real world problems are problems that can be modelled with the use of i.i.d r.v's. \\
...
In the case of 2 discrete r.v's, straight from the definition (\ref{eq:jointpmf}), their joint probability mass function reads:
...
By making use of independent r.v's and (\ref{eq:independentjoint}) we can show the following relation for the joint expected value of independent r.v's.

\begin{lemma}  \label{eq:independentexpected}
...
\begin{align*} 
    E[X \cdot Y] &= \sum _{x} \sum _{y} x \cdot y \cdot P_{X,Y}(X=x, Y=y) \\
           ...
           &= \Big( \sum _{x} x  P_{X}(X=x) \Big) \Big(\sum _{y} y P_{Y}(Y=y) \Big) \\
           &= E[X] E[Y]
\end{align*}
\end{proof}
\end{lemma}

The joint expected value can be generalized to $E[g(X,Y)]$ for any function $g$ of the random variables $X$ and $Y$. In case where $g(X,Y) = g_{X}(X) \cdot g_{Y}(Y)$ then for independent events also holds $E[g(X,Y)] = E[g_{X}(X)] E[g_{Y}(Y)]$. A generalization to a function $g$ for any number of r.v's is also possible. \\
...
A similar formula as (\ref{eq:variance}) for variance can also be derived for covariance:
...
In case of independent events, by making use of lemma (\ref{eq:independentexpected}) we get for the covariance of independent events:
\begin{align*}
...
\begin{align} \label{eq:variance_covariance}
    Var(X + Y) &= Cov(X+Y, X+Y) \nonumber \\
               &= Cov(X+Y, X) + Cov(X+Y, Y) \nonumber \\
...
By using both (\ref{eq:variance}) and (\ref{eq:covariance}) and the definition of standard deviation being the square root of variance follows:
...
Two of the most important properties of correlation is that it carries no units and it is always between -1 and 1. Let's show that!
...
\begin{proof}
Without loss of generality we can assume that $X$ and $Y$ are standardized r.v's (i.e $E[X] = E[Y] = 0$ and $Var(X) = Var(Y) = 1$). In this case $Cov(X,Y) = \rho(X,Y)$. Then by (\ref{eq:variance_covariance}) we get:
\begin{align*}
...
\begin{figure}[H]
\includegraphics[scale=0.65]{images/correlation.png}
\centering
\caption {Correlation Between Two Random Variables}
\end{figure}

\vspace{10pt}

As a final note, we can in a similar way define everything for the case of continuous r.v's. Namely, straight from (\ref{eq:jointpdf}) their joint probability density function will read:
...
Hence $X$ follows a Bern\Big($\frac{1}{2}$\Big). 
...
Hence $Y$ follows a Bern\Big($\frac{1}{3}$\Big).\\
...
Since they are independent we can use lemma (\ref{eq:independentexpected}) to calculate the joint expected value:
\begin{align*}
...
\begin{figure}[H]
\includegraphics[scale=0.16]{images/skewness.png}
\centering
\caption {Skewness Of A Random Variable $X$}
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale=0.66]{images/kurtosis.png}
\centering
\caption {Kurtosis Of A Random Variable $X$}
\end{figure}
...
\centering
\caption {Moments Of A Random Variable $X$}
\end{figure}
...
\begin{align*}
M_{X} (t) &= E[e^{tX}] \\
			  &= E \Big[ 1 + tX + \frac{t^{2}X^{2}}{2!} + \frac{t^{3}X^{3}}{3!} + \ldots + \frac{t^{n}X^{n}}{n!} +\ldots \Big] && \text{(Taylor expansion of $e^{tX}$)} \\
			  ...
			  &= 1 + t \mu_{1} + \frac{t^{2}\mu_{2}}{2!} + \frac{t^{3}\mu_{3}}{3!}   + \ldots +  \frac{t^{n}\mu_{n}}{n!}  + \ldots && \text{($\mu_n = E[X^n]$)}
\end{align*}}

Moment generating function is really importnat since by differentiating it $i$ times with respect to $t$ and setting $t=0$, we obtain the i-th raw moment. Also we can determine probability distributions since if two r.v's have the same moment generating function that means that they follow the same probability distribution.\\
...
M_{X} (t) &= E[e^{tX}] \\
			   ...
			   &= \frac{1}{\sqrt{2\pi\sigma^2}}  \cdot exp \Big( - \frac{\mu^2}{2\sigma^2} \Big) \cdot exp \Big( \frac{(\sigma^2t +\mu)^2}{2 \sigma^2} \Big) \int_{- \infty}^{\infty} exp \Big(- \frac{1}{2\sigma^2} \Big( x^2 - 2x (\sigma^2t +\mu) + (\sigma^2t +\mu)^2  \Big) \Big)  dx \\
			    ...
			    &= \frac{1}{\sqrt{2\pi\sigma^2}} \cdot exp \Big( \frac{ -\mu^2 + \sigma^4 t^2 + 2 \sigma^2 t \mu +\mu^2}{2 \sigma^2} \Big)  \cdot (\sqrt{2\pi} \sigma) \\
			    	...
			    	&= exp \Big( \frac{ 2\sigma^2 (\frac{1}{2}\sigma^2 t^2 + t \mu}{2 \sigma^2} \Big) \\
