...
\bd [Push-Forward]
Let $\phi\cl M \to N$ be a smooth map between smooth manifolds. The \textbf{push-forward}\index{push-forward} (or \textbf{derivative}) of $\phi$ at $p\in M$ is the linear map:
\bi{rrCl}
...
In other words, since $\phi_{*}$ is a map from one tangent space to another this means that it acts on a tangent vector and produces another one, hence $\phi_{*} (X)$ is again a tangent vector (but on $N$). As a tangent vector it can act on a smooth function (again on N) and produce a real number, hence the action of a push-forward on a function is simply
\bi{rrCl}
...
\bd [Commutator]
Let $M$ be a smooth manifold and let $X,Y\in\Gamma(TM)$. The \textbf{commutator} (or \textbf{Lie bracket}\index{Lie bracket}) of $X$ and $Y$ is defined as
\bi{rrCl}
...
Using this definition we can now extend the gradient as follows:
