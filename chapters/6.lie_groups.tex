...
\ell_g(hh') \neq \ell_g(h)\,\ell_g(h')
\ese
in general. However that does not stop $\ell_g$ from being a diffeomorphism of the underlying manifolds.\\
...
We can go one step further now. Recall from the Lie algebra chapter in the notes, that a Lie algebra over an algebraic field $K$ is a vector space over $K$ equipped with a Lie bracket $[-,-]$, i.e.\ a $K$-bilinear, antisymmetric map which satisfies the Jacobi identity. \\
...
By inheriting the commutator to the finite dimensional $R$-vector space $\mathcal{L}(G)$, we can then turn $(\mathcal{L}(G),+,\cdot,[-,-])$ to a subalgebra of $\Gamma(TG)$ (of course we need to show that $\mathcal{L}(G)$ is closed under the commutator).
...
\subsubsection{The Lie Algebra Of \texorpdfstring{$\SL(2,\C)$}{SL(2,C)}}
