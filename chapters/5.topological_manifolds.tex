...
where $X$ is a vector field and when acted on a point $p$ produces the corresponding vector $X_p$. (From time to time, when we will need to apply the definition of a vector, we will remind ourselves that $X_p$ actually means $X_{\gamma, p}$ so we are not confused from the $\gamma$ in the definition).
