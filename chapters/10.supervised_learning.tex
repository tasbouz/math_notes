...
\begin{figure}[H]
\includegraphics[scale=0.55]{images/supervisedmodel.png}
\centering
...
\begin{definition}[Simple \& Multiple Linear Regression]
...
\begin{definition}[Univariate \& Multivariate Linear Regression]
...
\begin{figure}[H]
\includegraphics[scale=0.35]{images/linearregression.png}
\centering
...
It is a usual procedure to define $x_0 = 1$, so (\ref{eq:linearregression}) can be rewritten as:
...
then we can rewrite (\ref{eq:linearregression}) in a very simple form of:
...
\begin{equation} \label{eq:errorslinear}
...
From (\ref{eq:errorslinear}):
\begin{align*} \label{eq:errorslinear}
...
According to the principle of maximum likelihood, the best parameters can be found by maximizing the log-likelihood. The first term of the log-likelihood is just a constant term so it does not contribute at all to the maximization, and the same holds for the denominator of the second term. \\
...
\begin{definition} [Mean Squared Error Loss Function]
...
\begin{equation} \label{eq:lrloss}
...
\begin{definition} [Optimization Techniques]
...
\begin{figure}[H]
\includegraphics[scale=0.35]{images/gradientdescent.png}
\centering
...
\begin{figure}[H]
\includegraphics[scale=0.95]{images/alpha.png}
\centering
...
\begin{equation} \label{eq:gradientdecentlinear}
...
Batch gradient descent is actually the one we just saw. As we see in (\ref{eq:gradientdecentlinear}), the whole training set $X$ is used in order to make just one update of the parameters. We usually refer to the whole training set as the \say{batch}. For that reason, the usual terminology for what we have seen so far is  \say{batch gradient descent}, meaning that the whole batch is used to update the parameter. 
...
\textit{(From now on, in order to save space, we will write $h(\boldsymbol{x})$ instead of the actual expression for the sigmoid function (\ref{eq:sigmoid}))}.
...
where again we make the assumption that we are dealing with independent and identically distributed random variables.\\
...
Once again, according to the principle of maximum likelihood, the best parameters can be found by maximizing the log-likelihood.\\
...
The only way for solving the optimization problem is through gradient descent.

\subsection{Gradient Descent}

Gradient descent  works fine in logistic regression, using again the rule (\ref{eq:gradientdescent}) for updating the weights. First, let's calculate the derivative of $J(\boldsymbol{w})$ for logistic regression.
...
\begin{definition}[Point-Wise Error]
...
\begin{definition}[Overall Error]
...
\begin{figure}[H]
\includegraphics[scale=0.6]{images/bias.png}
\centering
\caption {High Bias (Red = Hypothesis Function, Blue Points = Dataset)}
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale=1.1]{underfitting.png}
\centering
...
\begin{figure}[H]
\includegraphics[scale=0.6]{variance.png}
\centering
\caption {High Variance (Red = Hypothesis Function, Blue Points = Dataset)}
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale=1.2]{overfitting.png}
\centering
\caption {Overfitting}
\end{figure}
...
From now on we will refer to the first expression as $J_{\text{train}} = E_{in}$ and to the second one as $J_{\text{eval}} = E_{out}$.\\
...
\begin{figure}[H]
\includegraphics[scale=0.45]{eval.png}
\centering
\caption {Detecting Underfiting And Overfitting}
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale=0.57]{lchighbias.png}
\centering
\caption {Learning Curves - Underfitting}
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale=0.6]{lchighvariance.png}
\centering
\caption {Learning Curves - Overfitting}
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale=0.1]{regularization.png}
\centering
\caption {Overfitting}
\end{figure}
...
Of course, L2 regularization can be applied also for the case of logistic regression. More specifically, for logistic regression where $J(\boldsymbol{w})$ is given by(\ref{eq:logrloss}) the Lagrangian reads:
...
\begin{definition}[True Positive]
...
\begin{definition}[True Negative]
...
\begin{definition}[True Negative Rate]
...
\begin{definition}[Positive Predicted Value]
...
\begin{definition}[Negative Predicted Value]
...
\begin{definition}[False Omission Rate]
...
\begin{definition}[Null Error Rate]
