...
Notice that in the end,  a feedforward neural network is simply another supervised model as the ones we developed in the supervised learning chapter. Hence everything we developed there holds also for a feedforward neural network (i.e: error analysis,  bias variance trade-off,  solutions to undefitting / overfitting, etc).  \v
...
Notice that this section introduces no new concepts, but a handy way of combining together all the training examples in matrices. The usefulness of this more practical than theoretical since once someone wants to use programming to actually train a model switching to vectors and matrices is inevitable.
...
\bse
{\boldsymbol{h}^{[l]}}, \boldsymbol{\delta^{[l]}} \sim \boldsymbol{x}
\ese
